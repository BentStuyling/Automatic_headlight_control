...
void gyroCarAngle() {
  ...
  // otherwise, check for DMP data ready interrupt (this should happen frequently)
      ...
      currentGyroCarAngle = ypr[1] * 180/M_PI; // 1 is yaw (z)
      lastGyroSignalTime = millis();         
  }
  
  // Serial.print(F("Current gyro  angle [deg] = ")); Serial.println(currentGyroCarAngle);  
  Serial.print(F(">Current gyro angle [deg]:")); Serial.println(currentGyroCarAngle);
  // Set limits to output values in case of faulty measurement
  ...
  if(currentGyroCarAngle == 0.0){
    Serial.print(F("Current gyro angle excaclty 0 (faulty measurement): "));Serial.println(currentGyroCarAngle);  
    currentGyroCarAngle= smoothedGyroCarAngleSignal;
  }
  else if(abs(currentGyroCarAngle-smoothedGyroCarAngleSignal)>5 and i>99 ){
...
void adjustmentrun() {                     
  ...
  if((millis()- starttime)< 35000){                                                                     // Start using gyro ofsset only after 35s of calibration of the gyro.
    GyroCarAngleOffset = 0;                                                                     // Before that time the gyro offset is gero.
  }
  float requiredHeadlightCorrectionInmm = headlightAngleToStroke(CarAngleOffset-GyroCarAngleOffset);                        //Calculate the required stepper stroke to correct for the current car angle offset, positive means the headlight needs to move down.
  float newStepperPositionGoal = defaultStepperposition - requiredHeadlightCorrectionInmm;
  
  Serial.print(F(">Smoothed rear angle [deg]:")); Serial.println(smoothedRearCarAngle); 
  Serial.print(F(">Smoothed gyro angle [deg]:")); Serial.println(smoothedGyroCarAngleSignal); 
  Serial.print(F(">Floating gyro angle [deg]:")); Serial.println(floatingOffsetGyroCarAngle); 
  
  
  // Serial.print(F("Smoothed gyro car angle offset [deg] = ")); Serial.print(smoothedGyroCarAngleOffset); 
  // Serial.print(F(" Smoothed rear car angle [deg] = ")); Serial.println(smoothedRearCarAngle);  
  // Serial.print(F(", offset (non gyro) [deg] = "));  Serial.println(currentCarAngleOffset);
  ...
  if(requiredStepSize >= minimumStepsize){
    ...
    Serial.print(F("New stepper position [mm] = ")); Serial.println(stepper.getCurrentPositionInMillimeters());
  }
  else{
  ...
  Serial.print(F(">Stepper position[mm]:")); Serial.println(newStepperPositionGoal);
}
...
void setup() {
  ...
  smoothedRearCarAngle = currentRearCarAngle; // pre-initialize smoothed signal
  starttime = millis();
...
void loop() {
  ...
  if((currentLoopTime - lastAdjustmentrun) >adjustmentInterval){
    adjustmentrun(); 
    mpu.resetFIFO();                                                                                                 // Run 1 adjustment 
