...
const float ewmaAlphaRearCarAngle = 0.005;                                                                           // The EWMA alpha value (α) for smoothing (smaller is more smoothing) [-]
...
int GyroAction = 0;                                                                                                  // set activity of gyro (0= 0ff)
int starttime = 0;                                                                                                   // Timersignal used for disregarding the gyro measurements the first x seconds
...
void gyroCarAngle() {
  ...
  // otherwise, check for DMP data ready interrupt (this should happen frequently)
      ...
      currentGyroCarAngle = ypr[0] * 180/M_PI; // 0 is yaw (z)
  ...
  else if(currentGyroCarAngle == 0.0){
...
void adjustmentrun() {                     
  ...
  Serial.print(F(">CrntGyASlow:")); Serial.println(currentGyroCarAngle);
